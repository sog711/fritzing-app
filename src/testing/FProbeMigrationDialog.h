...
// object name (partMigrationDialog, migrationOldRadio, migrationNewRadio, migrationSilenceButton,
// migrationUpdateAllButton, migrationPrevButton, migrationNextButton, plus the labels), so the
// probe needs no access to MigrationHandler internals.
//   read()  -> JSON describing the current dialog state (open, selected version, labels, buttons)
//   write() -> an action string: "new", "old", "silence", "updateAll", "next", "previous", "close"
